...
headers_train, ratings_train = load_csv(os.path.join(current_path, './Amazon Movies/tsv_data/train.tsv'))
headers_test, ratings_test = load_csv(os.path.join(current_path, './Amazon Movies/tsv_data/test.tsv'))
df_user_profile = pd.read_csv(os.path.join(current_path, './Amazon Movies/meta_data/user.meta'), sep = '	', header = None)

group_dict, feature_values = get_user_group_info(os.path.join(current_path, './Amazon Movies/tsv_data/multicore.tsv'))
...
#print(item_frequent_dict_train)
item_id_list = [e[0] for e in item_frequent_dict_train[:num_items]]

#print(item_id_list)
user_id_list = sorted(set([e[0] for e in ratings_train]), key=lambda x:x)[:num_users]
random.seed(10)
random.shuffle(user_id_list)

#Client sampling 
user_id_list = user_id_list[:4947]
...
for record in ratings_train:
    ...
    if record[0] not in user_id_list or record[1] not in item_id_list:
        continue
    counter += 1
    ratings_dict_train[record[0]].append([item_id_list.index(record[1]), float(record[2]), int(record[3])])
...
for item in train_data:
    train_data[item] = train_data[item][:20] 
...
item_id_list_test = [e[0] for e in item_frequent_dict_test[:len(item_frequent_dict_test)]]

user_id_list_test = sorted(set([e[0] for e in ratings_test]), key=lambda x:x)[:num_users]

random.seed(10)
random.shuffle(user_id_list_test)
user_id_list_test = user_id_list_test[:4947]
...
for i in user_id_list_test:
    if df_user_profile.loc[df_user_profile[0] == i, 1].iloc[0] == 'active':
...
for record in ratings_test:
    ...
    if record[0] not in user_id_list_test or record[1] not in item_id_list_test:
        continue
    counter += 1
    ratings_dict_test[record[0]].append([item_id_list_test.index(record[1]), float(record[2]), int(record[3])])
...
for record in ratings_test:
    ...
    if record[0] not in user_id_list_test_male or record[1] not in item_id_list_test:
        continue
    counter += 1
    ratings_dict_test_male[record[0]].append([item_id_list_test.index(record[1]), float(record[2]), int(record[3])])
...
for record in ratings_test:
    ...
    if record[0] not in user_id_list_test_female or record[1] not in item_id_list_test:
        continue
    counter += 1
    ratings_dict_test_female[record[0]].append([item_id_list_test.index(record[1]), float(record[2]), int(record[3])])
